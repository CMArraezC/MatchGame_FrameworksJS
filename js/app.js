...
{
  colorTitulo(); //Para que se ejecure el cambio de color al título al cargar la pagina
  });

//Al hacer Click en el Boton "iniciar" se ejecutan...
...
//Función para que el título cambie de color
function colorTitulo() {
  ...
  div.animate({color: color1}, 1000, function()
  {
    ...
    {
      div.animate({color: color3}, 1000, function()
      {
      colorTitulo();
      });
    });
	...
	if(i<8){
		for(var j=1;j<8;j++){
			if($(".col-"+j).children("img:nth-child("+i+")").html()==null){
				...
				$(".col-"+j).prepend("<img src='image/"+numero+".png' class='elemento'/>").css("justify-content","flex-start")
			}}}
	if(i==8){
	clearInterval(intervalo);
...
function timer(){
	...
	if(seg==0){
		if(min==0){
			...
			clearInterval(intervalo);
			clearInterval(tiempo);
			$(".panel-tablero").hide("drop","slow",termino);
			$(".time").hide();}
		seg=59;
		min=min-1;}
...
function eliminarhorver(){
	...
	if(busquedaHorizontal==1||busquedaVertical==1){
		...
		$(".activo").hide("pulsate",200,function(){
